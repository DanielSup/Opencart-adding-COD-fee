<?php
class ModelShippingZasilkovnaSK extends Model {
  function getQuote($address) {
    $this->load->language('shipping/ZasilkovnaSK');
    $weight = $this->cart->getWeight();
    $max_weight = $this->config->get('ZasilkovnaSK_weight_max');
    $valid_weight = (!$max_weight && $max_weight !== 0) || ($max_weight > 0 && $weight <= $max_weight); // weight condition check, yay logic
    if ($this->config->get('ZasilkovnaSK_status') && $valid_weight) {
          $query = $this->db->query("SELECT * FROM " . DB_PREFIX . "zone_to_geo_zone WHERE geo_zone_id = '" . (int)$this->config->get('ZasilkovnaSK_geo_zone_id') . "' AND country_id = '" . (int)$address['country_id'] . "' AND (zone_id = '" . (int)$address['zone_id'] . "' OR zone_id = '0')");
    
          if (!$this->config->get('ZasilkovnaSK_geo_zone_id')) {
            $status = TRUE;
    ...
    if ($status) {      
      ...
      $api_key = $this->config->get('ZasilkovnaSK_api_key') ;
...
function setRequiredOpt(){
        ...
        var ZasilkovnaSK_selected = false;
        var opts={
...
function updateConnectedField(opts, id) {
    ...
    function addHooks(){
      //called when no ZasilkovnaSK method is selected. Dunno how to call this from the branch.js        
...
$("#content").delegate("textarea[name=comment]", "change", function () {
  updateConnectedField();
      ...
      $addedHelperJS = false;
      for($i = 0; $i <= 10 ;$i++){
        $enabled = $this->config->get('ZasilkovnaSK_enabled_'.$i);
        if (empty($enabled) || $enabled == 0) continue;

    $cost = 0;
    if($this->config->get('ZasilkovnaSK_freeover_'.$i) == 0 || $this->cart->getTotal() < $this->config->get('ZasilkovnaSK_freeover_'.$i)) // shipment is not free
      $cost = $this->config->get('ZasilkovnaSK_price_'.$i);
    
        $title = $this->config->get('ZasilkovnaSK_title_'.$i);                
        $country = $this->config->get('ZasilkovnaSK_destination_'.$i);                
        ...
        if($this->config->get('ZasilkovnaSK_branches_enabled_'.$i)){
          $JS .= '<script>                
            var radio = $(\'input:radio[name="shipping_method"][value="ZasilkovnaSK.'.$title.$i.'"]\');
            var td = radio.parents("td").next(); 
            if(td.find(\'#ZasilkovnaSK_box\').length == 0){
              $(td).append(\'<div id="ZasilkovnaSK_box" class="packetery-branch-list list-type=8 connect-field=textarea[name=comment] country='.$country.'" style="border: 1px dotted black;">Načítání: seznam poboček osobního odběru</div> \');                                      
        ...
        $quote_data[$title.$i] = array(
          'id'            => 'ZasilkovnaSK.'.$title.$i,
          'code'            => 'ZasilkovnaSK.'.$title.$i,
          'title'           => $this->language->get('text_description'),
          'cost'            => $cost,
          'tax_class_id'    => $this->config->get('ZasilkovnaSK_tax_class_id'),
		  'cod_fee'      => 21,
          'text'            => $JS.$this->currency->format($this->tax->calculate($cost, $this->config->get('ZasilkovnaSK_tax_class_id'), $this->config->get('config_tax')))
        );        
      }      
      ...
      $method_data = array(
        ...
        'quote'      => $quote_data,
        'sort_order' => $this->config->get('ZasilkovnaSK_sort_order'),
		'cod_fee'      => 21,
        'error'      => FALSE
      );
    }    
